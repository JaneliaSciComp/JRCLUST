function corScores = clusterWaveformSim(meanWfSet, clusterSites, hCfg, scoreData, iCluster)
    ...
    if fUsePeak2
        ...
        compClusters = find(ismember(sites, findNearSite_(hCfg.siteLoc, iSite, hCfg.evtDetectRad)));
    end
...
function maxCor = maxCorPearson(iWaveforms, jWaveforms, shifts1, shifts2, fMode_cor)
    assert_(numel(iWaveforms) == numel(jWaveforms), 'maxCor_drift_: numel must be the same');

...
if obj.hCfg.getOr('testRun', 0)
    preSplit(splitDlg, nsplit.String, btngrp, hFigSplit);
else
    uiwait(splitDlg);
end
...
while 1
    ...
    % for automatic testing, take the default split 
    if obj.hCfg.getOr('testRun', 0)
        finishSplit(hFigSplit, 0, 1);
    else % ...otherwise, wait for user input
...
if numel(partitioning) > 1
    success = obj.splitCluster(iCluster, partitioning);
end
end %fun
...
function finishSplit(hFigSplit, fAbort, noConfirm)
if nargin < 3
    noConfirm = 0;
end

function success = partitionCluster(obj, iCluster, assignPart)
    ...
    for iSplit = 1:numel(assignPart)-1
        ...
        obj.clusterCenters(iCluster + iSplit) = splitOff(splitCenter);
        % in case we took iCluster's center with us
        if obj.clusterCenters(iCluster) == obj.clusterCenters(iCluster + iSplit)
            [~, retainCenter] = max(obj.spikeRho(retain));
            obj.clusterCenters(iCluster) = retain(retainCenter);
        end
    end

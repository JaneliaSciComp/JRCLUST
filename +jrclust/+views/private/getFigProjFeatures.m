function dispFeatures = getFigProjFeatures(hClust, sitesToShow, selected)
    ...
    if strcmp(dispFeature, 'pca')
        ...
        if ~isempty(jCluster)
            fgWindows2 = permute(hClust.getSpikeWindows(fgSpikes2, sitesToShow, 0, 0), [1, 3, 2]); % nSamples x nSpikes x nSites
    ...
    elseif strcmp(dispFeature, 'vpp') % minimum vs maximum voltage
        ...
        if ~isempty(jCluster)
            fgWindows2 = hClust.getSpikeWindows(fgSpikes2, sitesToShow, 0, 1); % use voltages

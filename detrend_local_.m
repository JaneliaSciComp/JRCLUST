...
function [clusterCenters, x, z] = detrend_local_(S_clu, P, fLocal)
    ...
    S0 = get0_();
    % cvi_rho_rank_site = cellfun(@(vi)rankOrder(S_clu.rho(vi), 'ascend'), S0.cviSpk_site, 'UniformOutput', 0);
    % cvi_delta_rank_site = cellfun(@(vi)rankOrder(S_clu.delta(vi), 'ascend'), S0.cviSpk_site, 'UniformOutput', 0);

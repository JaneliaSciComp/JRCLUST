%--------------------------------------------------------------------------
function [hFig, figData] = plotFigClusterCor(S0)
    if nargin < 1
        S0 = get(0, 'UserData');
    end
    ...
    if isempty(figData)
        ...
        figData.hImWavCor = imagesc(S_clu.mrWavCor, P.corrLim); %clears title and current figure
        ...
        set(hFig, 'KeyPressFcn', @keyPressFcn_FigClusterCor_);

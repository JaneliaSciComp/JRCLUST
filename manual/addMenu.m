...
function addMenu(hFig, P)
    ...
    if getOr(P, 'fImportKilosort', 0) && get_(P, 'fWav_raw_show')
        uimenu(mh_view,'Label', 'Show templates', 'Callback', @(h,e) showTemplates(h), ...
            'Checked', ifeq_(get_(P, 'fshowTemplates'), 'on', 'off'));
    end

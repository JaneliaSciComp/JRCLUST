...
function plotFigTime(S0)
    ...
    [vrFet0, vrTime0] = getFigTimeFeatures(primarySite, [], S0); % plot background
    [vrFet1, vrTime1, vcYlabel, viSpk1] = getFigTimeFeatures(primarySite, S0.primarySelectedCluster, S0); % plot primarySelectedCluster
    ...
    % iFet = figData.iFet;
    % iFet = 1;
    if ~isfield(figData, 'primarySite'), figData.primarySite = []; end
    updatePlot(figData.hPlotBG, vrTime0, vrFet0);
    updatePlot(figData.hPlotFG, vrTime1, vrFet1);
    ...
    if ~isfield(figData, 'fPlot0'), figData.fPlot0 = 1; end

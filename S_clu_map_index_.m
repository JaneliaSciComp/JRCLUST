...
function S_clu = S_clu_map_index_(S_clu, viMap_clu)
    ...
    S_clu.cviSpk_clu = arrayfun(@(iClu)find(S_clu.viClu==iClu), 1:S_clu.nClusters, 'UniformOutput', 0);
    S_clu.vnSpk_clu = cellfun(@numel, S_clu.cviSpk_clu);
    viSite_spk = get0_('viSite_spk');
    S_clu.viSite_clu = double(arrayfun(@(iClu)mode(viSite_spk(S_clu.cviSpk_clu{iClu})), 1:S_clu.nClusters));
end %func

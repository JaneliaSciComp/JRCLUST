%--------------------------------------------------------------------------
function csFiles_bin = dir_files_(multiFilenames, vcFile_txt, file_sort_merge)
    ...
    if isempty(multiFilenames)
        fprintf('No files to merge ("multiFilenames" is empty).\n');
        csFiles_bin = {}; return;
    end
    % fprintf('Listing files to merge ("multiFilenames"):\n');
    csFiles_bin = filter_files_(multiFilenames, file_sort_merge);
    if nargout==0

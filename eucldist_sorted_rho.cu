 ...
 */
__global__ void eucldist_sorted_rho(float const *A, unsigned int *D, int const nA, int const nneigh, int const nC, float const dc){
    int iA = (blockIdx.x + blockIdx.y * gridDim.x) * CHUNK;    
    int tx = threadIdx.x;
    __shared__ float sA[NC][CHUNK];
    __shared__ unsigned int rho1[NTHREADS][CHUNK];
    ...
    for (int i=0; i<CHUNK; ++i) rho1[tx][i] = 0;
    __syncthreads();
    ...
    while (iB <= iB_max){
        ...
        for (int i=0; i<CHUNK; ++i){            
            if (dist[i] < dc2){
                if (iA+i < nA){
                    int dab = ABS(iA+i-iB);
                    if (dab<=nneigh || nneigh==0) ++rho1[tx][i];
                }
            }
        }
        iB += blockDim.x;

...
function S0 = ui_merge_(S0)
    ...
    if S0.primarySelectedCluster == S0.iCluPaste
        msgbox_('Cannot merge to itself.', 1); return;
    end

    figure_wait_(1); drawnow;
    S0.S_clu = merge_clu_(S0.S_clu, S0.primarySelectedCluster, S0.iCluPaste, P);
    set(0, 'UserData', S0);
    plotFigWav(S0); %redraw plot
    S0.primarySelectedCluster = min(S0.primarySelectedCluster, S0.iCluPaste);
    S0.iCluPaste = [];
    set(0, 'UserData', S0);
    update_plot_(S0.hPaste, nan, nan);
    S0 = update_FigCor_(S0);
    S0 = button_CluWav_simulate_(S0.primarySelectedCluster, [], S0);
    S0 = save_log_(sprintf('merge %d %d', S0.primarySelectedCluster, S0.iCluPaste), S0);
    set(0, 'UserData', S0);

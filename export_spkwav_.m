...
function export_spkwav_(P, vcArg2, fDiff)
    ...
    for iClu = viClu
        tnWav_clu1 = getSpikeWaveformsSites(S_clu.spikesByCluster{iClu}, miSite_clu(:,iClu), S0);
        if fDiff
            ctrWav_clu{iClu} = tnWav_clu1;
        else
            ctrWav_clu{iClu} = tnWav2uV_(tnWav_clu1, P);
        end
        %     ctrWav_clu{iClu} = (cumsum(bit2uV_(meanSubtract(tnWav_clu1))));
        %     ctrWav_clu{iClu} = (bit2uV_(meanSubtract(tnWav_clu1)));
        fprintf('.');
    end
    fprintf('\n\ttook %0.1fs\n', toc(t1));
    ...
    if fPlot
        ...
        %     if getOr([], 'fDebug_ui', 0), tryClose(hFig); end
        if fDebug_ui==1, tryClose(hFig); end
    else

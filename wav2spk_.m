%--------------------------------------------------------------------------
function [tnWav_spk_raw, tnWav_spk, trFet_spk, miSite_spk, spikeTimes, vnAmp_spk, vnThresh_site, useGPU] = ...
    wav2spk_(mnWav1, vrWav_mean1, P, spikeTimes, viSite_spk, mnWav1_pre, mnWav1_post)
    ...
    if isempty(spikeTimes) || isempty(viSite_spk)
        P_ = setfield(P, 'nPad_pre', nPad_pre);
        [spikeTimes, vnAmp_spk, viSite_spk] = detect_spikes_(mnWav3, vnThresh_site, vlKeep_ref, P_);
    else
        spikeTimes = spikeTimes + nPad_pre;
        vnAmp_spk = mnWav3(sub2ind(size(mnWav3), spikeTimes, viSite_spk)); % @TODO read spikes at the site and time
    end
    ...
    if ~isempty(mnWav1_pre) || ~isempty(mnWav1_post)
        ...
        [spikeTimes, vnAmp_spk, viSite_spk] = multifun_(@(x)x(viKeep_spk), spikeTimes, vnAmp_spk, viSite_spk);
    end %if
    ...
    %     tnWav_spk = mn2tn_wav_spk2_(mnWav2, viSite_spk, spikeTimes, P);
    ...
    [tnWav_spk_raw, tnWav_spk, spikeTimes] = mn2tn_wav_(mnWav1, mnWav2, viSite_spk_, spikeTimes, P); fprintf('.');
    if nFet_use >= 2
    ...
    if get_set_(P, 'fCancel_overlap', 0)
        try
            [tnWav_spk, tnWav_spk2] = cancel_overlap_spk_(tnWav_spk, tnWav_spk2, spikeTimes, viSite_spk, viSite2_spk, vnThresh_site, P);
        catch

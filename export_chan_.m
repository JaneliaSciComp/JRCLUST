...
function export_chan_(P, vcArg1)
    ...
    try
        ...
        vcFile_out = strrep(P.paramFile, '.prm', sprintf('_ch%s.jrc', vcChan_(1:end-1)));
        ...
        for iChan1 = 1:numel(viChan)
            iChan = viChan(iChan1);
            vcFile_out = strrep(P.paramFile, '.prm', sprintf('_ch%d.jrc', iChan));
            fprintf('Loading chan %d(%d/%d) from %s\n\t', iChan, iChan1, numel(viChan), P.paramFile);
            t1 = tic;

function plot_ripple_hist(hFile, port, headstage, channel, varargin)
%PLOT_RIPPLE_HIST Plot spike data using Grapevine port ids
% that translate easily to Ripple Grapevine NIP ports and headstages.
% 
% Usage: ns_RESULT = plot_ripple_hist(hFile, port, headstage, channel)
%   Parameters:
...
index = entity.index;
if ~isempty(varargin)
    spike_hist(hFile, index, 1, varargin{1});
else
    spike_hist(hFile, index, 1);
end

%--------------------------------------------------------------------------
function [mrFet12_, viSpk12_, n1_, n2_, viiSpk12_ord_] = fet12_site_(spikeFeatures, S0, P, iSite, vlRedo_spk)
    % decide whether to use 1, 2, or 3 features
    if nargin<5, vlRedo_spk = []; end
    nFet_use = getOr(P, 'nFet_use', 2);
    
    time_feature_factor = getOr(P, 'time_feature_factor', 0); % TW
    ...
    switch nFet_use
        case 3
            ...
            mrFet12_ = [squeeze_(spikeFeatures(:,1,viSpk1_),2), squeeze_(spikeFeatures(:,2,viSpk2_),2), squeeze_(spikeFeatures(:,3,viSpk3_),2)];
            viSpk12_ = [viSpk1_; viSpk2_; viSpk3_];
            [n1_, n2_] = deal(numel(viSpk1_), numel(viSpk2_) + numel(viSpk3_));
        case 2
            mrFet12_ = [squeeze_(spikeFeatures(:,1,viSpk1_),2), squeeze_(spikeFeatures(:,2,viSpk2_),2); single(S0.spikeTimes([viSpk1_;viSpk2_]))']; % TW
        ...
        case 1
            mrFet12_ = [squeeze_(spikeFeatures(:,1,viSpk1_),2); single(S0.spikeTimes(viSpk1_))']; % TW
            mrFet12_(end, :) = time_feature_factor*std(mrFet12_(1, :)).*mrFet12_(end, :)./std(mrFet12_(end, :)); % TW
            viSpk12_ = viSpk1_;
            [n1_, n2_] = deal(numel(viSpk1_), numel(viSpk2_));
    end
    ...
    try
        ...
        if getOr(P, 'fSpatialMask_clu', 1) && nSites_fet >= getOr(P, 'min_sites_mask', 5)
